...
class OpenCVAnimOperator(bpy.types.Operator):
    ...
    landmark_model_path = "lbfmodel.yaml"  # This file need to be located on the same folder
                ...
                for mark in landmarks:
                    ...
                    if Mouth_total_ratio < 0.25:  # if Mouth_total_ratio is close to zero, that means mouth is closed.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "OpenBeak"].value = 0  # mouth_open_shape_key is not activated so mouth is closed (mouth is closed)
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "OpenBeak"].value = 1  # mouth_open_shape_key is activated (open mouth)

                    difference_eyebrow1 = numpy.linalg.norm(
                        shape[23] - shape[43])  # euclidean distance between right eyebrow and eyelid points
                    difference_eyebrow2 = numpy.linalg.norm(
                        shape[24] - shape[44])  # euclidean distance between right eyebrow and eyelid points

                    difference_eyebrow3 = numpy.linalg.norm(shape[22] - shape[
                        26])  # euclidean distance between right eyebrow points, shape[26] = right corner of right eyebrow

                    Eyebrow_right_ratio = (difference_eyebrow1 + difference_eyebrow2) / (2 * difference_eyebrow3)

                    # print (Eyebrow_right_ratio)

                    if Eyebrow_right_ratio < 0.37:  # if Mouth_total_ratio is close to zero, that means mouth is closed.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp.L"].value = 1  # eye_brows_left_shape_key is  activated
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp.L"].value = 0  # eye_brows_left_shape_key is not activated

                    difference_eyebrow4 = numpy.linalg.norm(
                        shape[19] - shape[37])  # euclidean distance between left eyebrow and eyelid points
                    difference_eyebrow5 = numpy.linalg.norm(
                        shape[20] - shape[38])  # euclidean distance between left eyebrow and eyelid points

                    difference_eyebrow6 = numpy.linalg.norm(shape[17] - shape[
                        21])  # euclidean distance between left eyebrow points, shape[36] = left corner of left eyebrow

                    Eyebrow_left_ratio = (difference_eyebrow4 + difference_eyebrow5) / (2 * difference_eyebrow6)

                    # print (Eyebrow_left_ratio)

                    if Eyebrow_right_ratio < 0.37:  # if Mouth_total_ratio is close to zero, that means mouth is closed.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp.R"].value = 1  # eye_brows_right_shape_key is  activated
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp.R"].value = 0  # eye_brows_right_shape_key is not activated

                    Eyebrow_total_ratio = (Eyebrow_right_ratio + Eyebrow_left_ratio) / 2

                    # print (Eyebrow_total_ratio)

                    if Eyebrow_total_ratio < 0.3:  # Eyebrow_total_ratio is close to zero, that means eyebrows are close to eyes.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadDown"].value = 1  # Foreheaddown value is activated

                    elif 0.3 < Eyebrow_left_ratio < 0.4:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp"].value = 0  # forehead close to normal and Foreheadup shape_key is not activated

                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "ForeheadUp"].value = 1  # Foreheadup shape_key is activated
                        bpy.data.shape_keys["Key"].key_blocks["ForeheadDown"].value = 0
