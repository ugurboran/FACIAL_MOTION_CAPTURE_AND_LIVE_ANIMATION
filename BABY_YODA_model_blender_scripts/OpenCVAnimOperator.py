        ...
        if event.type == 'TIMER':
            ...
            if type(faces) is numpy.ndarray and faces.size > 0:
                ...
                for mark in landmarks:
                    ...
                    EAR_total_ratio = (EAR_ratio + EAR_ratio2) / 2 # Total eye aspect ratio formula, avarage of left and right eye aspect ratios
                    ...
                    if EAR_total_ratio < 0.25:  # if EAR_ratio is close to zero, that means eye blink happens
                        bpy.data.shape_keys["Key"].key_blocks[
                            "eye_blink"].value = 1  # eye_blink_shape_key is activated (eye blink happens)
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "eye_blink"].value = 0  # eye_blink_shape_key is not activated (eyes are open)
                    ...
                    if Mouth_total_ratio < 0.15:  # if Mouth_total_ratio is close to zero, that means mouth is closed.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "mouth_open"].value = 0  # mouth_open_shape_key is not activated so mouth is closed (mouth is closed)
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "mouth_open"].value = 1  # mouth_open_shape_key is activated (open mouth)

                    difference_eyebrow1 = numpy.linalg.norm(shape[23] - shape[43])
                    difference_eyebrow2 = numpy.linalg.norm(shape[24] - shape[44])

                    difference_eyebrow3 = numpy.linalg.norm(shape[22] - shape[26])
                    ...
                    if Eyebrow_total_ratio < 0.5:  # if Mouth_total_ratio is close to zero, that means mouth is closed.
                        bpy.data.shape_keys["Key"].key_blocks[
                            "eye_brows_up"].value = 0  # eye_brows_up_shape_key is not activated
                    else:
                        bpy.data.shape_keys["Key"].key_blocks[
                            "eye_brows_up"].value = 1  # mouth_open_shape_key is activated
